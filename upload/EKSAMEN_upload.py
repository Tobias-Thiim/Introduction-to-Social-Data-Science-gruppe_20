...
def prepare_ml_dataset_overall(real_estate_df):
    ...
    df_type9 = filtered_df[filtered_df['propertyType'] == 9]
    df_type10 = filtered_df[filtered_df['propertyType'] == 10]
    df_type11 = filtered_df[filtered_df['propertyType'] == 11]
    df_type12 = filtered_df[filtered_df['propertyType'] == 12]
    ...
    filtered_df = pd.concat([df_type1, df_type2, df_type3, df_type6, df_type7, df_type9, df_type10, df_type11, df_type12])
    ...
    filtered_df.loc[(filtered_df['lotSize'].isin([0, 1])) & (~filtered_df['propertyType'].isin([3,9])), 'lotSize'] = np.nan
    filtered_df.loc[(filtered_df['propertyType'].isin([3,9])), 'lotSize']=0
    ...
    ml_dataset = filtered_df[[
        'geometry', 'job_density', 'distance_to_nearest_station', 'nearest_station_name', 'departures_per_hour', 
        'distance_to_job_center', 'latitude', 'longitude', 'propertyType', 'energyClass', 'priceCash', 'selfsale', 
        'rooms', 'size', 'lotSize', 'buildYear', 'city', 'zipCode', 'area', 'exp', 'basementSize', 'businessArea', 
        'nonPremiumDiscrete', 'cleanStreet', 'municipality'
    ]]
    final_count = len(ml_dataset)
    print(f"Step 17: Select variables for machine learning. Final number of listings: {final_count}")

    #Step 18
    ml_dataset = ml_dataset[(ml_dataset['exp'] != 0) & (ml_dataset['exp'] <= 100000)]
    print(f"step 18 remove extreme exp: {len(ml_dataset)}")
   
    
 # Step 19: Drop rows only if there are missing values in the key columns
    ml_dataset = ml_dataset.dropna(subset=[
        'job_density', 'distance_to_nearest_station', 'departures_per_hour', 'distance_to_job_center', 
        'latitude', 'longitude', 'propertyType', 'energyClass', 'priceCash', 'rooms', 'size', 
        'lotSize', 'buildYear', 'exp', 'basementSize', 'municipality'
    ])
    print(f"Step 19: Drop rows with missing key values. We drop 9 engergyclass.  Final number of listings: {len(ml_dataset)}")
